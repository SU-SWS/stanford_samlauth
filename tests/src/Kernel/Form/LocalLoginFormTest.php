...
class LocalLoginFormTest extends KernelTestBase {
  ...
  public function testLoginFormConfig() {
    ...
    $this->assertFalse(\Drupal::config('stanford_samlauth.settings')
      ->get('hide_local_login'));
    $form = \Drupal::formBuilder()
      ->getForm('\Drupal\stanford_samlauth\Form\LocalLoginForm');

    $this->assertCount(26, $form);
    $form_state = new FormState();
    $form_state->setValue('hide_local_login', TRUE);
    \Drupal::formBuilder()
      ->submitForm('\Drupal\stanford_samlauth\Form\LocalLoginForm', $form_state);

    $this->assertTrue(\Drupal::config('stanford_samlauth.settings')
      ->get('hide_local_login'));
  }
